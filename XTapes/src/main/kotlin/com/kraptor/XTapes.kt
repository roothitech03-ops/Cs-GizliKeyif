...
class XTapes : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        // 1. Extract from iframes (Primary method)
        document.select("iframe[src]").forEach { iframe ->
            val src = fixUrl(iframe.attr("src"))
            // Filter out non-video iframes
            if (src.isNotEmpty() && !src.contains("recaptcha") && !src.contains("twitter") && !src.contains("izerv") && !src.contains("timbuk") && !src.contains("javascript:false")) {
                try {
                    // loadExtractor will handle common hosts like 88z.io, 74k.io if they are supported by CloudStream's core extractors
        ...
        // 2. Fallback: Manual extraction for common patterns if extractors fail
        ...
        for (pattern in patterns) {
            pattern.findAll(pageHtml).forEach { match ->
                ...
                if (videoUrl.contains(".mp4") || videoUrl.contains(".m3u8")) {
                    ...
                    callback.invoke(
                        newExtractorLink(this.name, this.name, videoUrl, data, Qualities.Unknown.value, linkType)
                    )
                    found = true
                }
            }
