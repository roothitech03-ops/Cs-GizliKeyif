...
class WatchAnimeWorldProvider : MainAPI() {
    ...
    private suspend fun extractAbyssLinks(
        ...
        try {
            ...
            // Helper to emit one intercepted URL (skips encrypted sssrr.org files)
            fun emitIfValid(rawUrl: String) {
                if (rawUrl.contains("sssrr.org")) return
            ...
            // Check primary intercept (GCS signed URL)
            val primary = resolved.first
            if (primary != null) emitIfValid(primary.url.toString())

            // Check secondary intercepts (m3u8 / mp4)
            for (req in resolved.second) emitIfValid(req.url.toString())
        ...
        if (!intercepted) {
            ...
            // If no extractor installed, silently produces no links
            // (better UX than a 3003 container error).
            try {
