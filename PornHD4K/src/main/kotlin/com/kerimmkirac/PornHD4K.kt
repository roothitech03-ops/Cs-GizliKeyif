...
class PornHD4K : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = document.select(".ml-item, .movie-item, .item").mapNotNull {
            it.toSearchResult()
        }
    ...
    private fun Element.toSearchResult(): SearchResponse? {
        val titleElement = this.selectFirst("a.ml-mask, .title a, a[title]") ?: return null
    ...
    override suspend fun search(query: String, page: Int): SearchResponseList {
        ...
        val document = app.get(url).document
        val searchResults = document.select(".ml-item, .movie-item, .item").mapNotNull {
            it.toSearchResult()
        }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val tags = document.select(".tags a, .genres a").map { it.text() }
        
        val recommendations = document.select(".ml-item, .movie-item, .item").mapNotNull {
            it.toSearchResult()
        }
