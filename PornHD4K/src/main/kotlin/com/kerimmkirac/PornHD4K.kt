...
class PornHD4K : MainAPI() {
    ...
    companion object {
        // Cookie key parts extracted from fixx.js (thfq6jcc6pj85tez)
    ...
    override val mainPage = mainPageOf(
        "$mainUrl/premium-porn-hd"           to "Latest Videos",
        "$mainUrl/category/brazzers"         to "Brazzers",
        "$mainUrl/studio/mylf"               to "MYLF",
        "$mainUrl/studio/blacked"            to "Blacked",
        "$mainUrl/studio/team-skeet"         to "TeamSkeet",
        "$mainUrl/studio/bangbros"           to "BangBros",
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = document.select("div.ml-item.item").mapNotNull {
            it.toSearchResult()
        }
    ...
    private fun Element.toSearchResult(): SearchResponse? {
        ...
        val img = this.selectFirst("img.thumb, img.mli-thumb, img")
    ...
    override suspend fun search(query: String, page: Int): SearchResponseList {
        ...
        val document = app.get(url).document
        val searchResults = document.select("div.ml-item.item").mapNotNull {
            it.toSearchResult()
        }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val tags       = document.select(".mvic-info a[href*='/tag/']").map { it.text().trim() }
        val allTags    = (categories + tags).distinct()

        val recommendations = document.select("div.ml-item.item").mapNotNull {
            it.toSearchResult()
        }
    ...
    override suspend fun loadLinks(
        ...
        if (episodeId.isNullOrBlank()) {
            Log.d("PornHD4K", "No episode ID found on page: $data")
            return false
        }

        Log.d("PornHD4K", "Found episode ID: $episodeId")
        ...
        Log.d("PornHD4K", "AJAX URL: $ajaxUrl")
        ...
        Log.d("PornHD4K", "AJAX response length: ${jsonText.length}")

        if (jsonText.length <= 1) {
            Log.d("PornHD4K", "AJAX returned empty/invalid response: $jsonText")
            return false
        }
        ...
        try {
            ...
            Log.d("PornHD4K", "Error parsing AJAX response: ${e.message}")
